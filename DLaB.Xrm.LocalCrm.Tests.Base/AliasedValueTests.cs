...
namespace DLaB.Xrm.LocalCrm.Tests
{
    ...
    public class AliasedValueTests : BaseTestClass
    {
        ...
        /// <summary>
        /// A joined entity's aliased name defaults to the EntityReferenceAttributeName.Attribute
        /// </summary>
        [TestMethod]
        public void LocalCrmTests_AliasedValue_MultipleAliasedEntitiesPostfixOccurence()
        {
            ...
            // Assert
            //
            var key = Incident.Fields.AccountId + "." + Incident.Fields.Description;
            ...
            key = Incident.Fields.IncidentId + "." + PhoneCall.Fields.Subject;
            Assert.IsTrue(account.Attributes.ContainsKey(key), $"Expected Aliased Attribute key {key} was not found.  Found {string.Join(", ", account.Attributes.Keys)}");
        }
        ...
        public void LocalCrmTests_AliasedValue_SingleUnaliasedEntity_Should_PostfixOccurance()
        {
            ...
            Assert.IsTrue(account.Attributes.ContainsKey(key), $"Expected Aliased Attribute key {key} was not found.  Found {string.Join(", ", account.Attributes.Keys)}");
            key = Incident.Fields.Id + "." + PhoneCall.Fields.Subject;
            Assert.IsTrue(account.Attributes.ContainsKey(key), $"Expected Aliased Attribute key {key} was not found.  Found {string.Join(", ", account.Attributes.Keys)}");
        }
